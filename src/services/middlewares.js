...
export function checkAccess(role, allowed) {
  return async (req, res, next) => {
    try {
      if (allowed.includes(req.user.access) && role.includes(req.user.role)) return next();
...
export async function socketAuth(socket, next) {
  try {
    const token = socket?.handshake?.headers?.cookie.split('=')[1]?.replace('; connect.sid', '') || socket?.handshake?.headers?.coredevs?.replace('Bearer ', '');
